...
const SettingsPropTypes = {
  ...
  eLocBounds: PropTypes.shape({
    eLocs: PropTypes.arrayOf(PropTypes.string).isRequired,
    /**
...
class Camera extends React.Component {
  ...
  _handleCameraChange(currentCamera, nextCamera) {
    ...
    if(nextCamera.eLocBounds && this._hasELocBoundsChanged(currentCamera, nextCamera)) {
      cameraConfig.eLocBounds = nextCamera.eLocBounds;
    } else {
      cameraConfig.centerELoc = nextCamera.centerELoc;
    }
  ...
  _hasELocBoundsChanged(currentCamera, nextCamera) {
    const cB = currentCamera.eLocBounds;
    const nB = nextCamera.eLocBounds;
   ...
   *
   * @example
   * this.camera.fitBoundsWithEloc([eloc])
   * this.camera.fitBoundsWithEloc([eloc], 20, 1000) // padding for all sides
   * this.camera.fitBoundsWithEloc([eloc], [verticalPadding, horizontalPadding], 1000)
   * this.camera.fitBoundsWithEloc([eloc], [top, right, bottom, left], 1000)
   *
   * @param {Array<String>} eLocBounds - North east coordinate of bound
   * @param {Number=} padding - Camera padding for bound
   * @param {Number=} animationDuration - Duration of camera animation
   * @return {void}
   */
   ...
   * this.camera.flyTo([lng, lat], 12000)
   *
   *  @param {Array<Number>} eLoc - Coordinates that map camera will jump too
   *  @param {Number=} animationDuration - Duration of camera animation
   *  @return {void}
   */
  flyWithEloc(eLoc, animationDuration = 2000) {
    return this.setCamera({
      centerELoc: eLoc,
      animationDuration,
      animationMode: Camera.Mode.Flight,
   ...
   *
   * @example
   * this.camera.moveTo("eloc", 200) // eases camera to new location based on duration
   * this.camera.moveTo("eloc") // snaps camera to new location without any easing
   *
   *  @param {String} coordinates - Coordinates that map camera will move too
   *  @param {Number=} animationDuration - Duration of camera animation
   *  @return {void}
   */
  moveWithEloc(eLoc, animationDuration = 0) {
    return this._setCamera({
      centerELoc: eLoc,
      animationDuration,
    });
  ...
  _createStopConfig(config = {}, ignoreFollowUserLocation = false) {
    ...
    if(config.centerELoc) {
      stopConfig.centerELoc = config.centerELoc;
    }
    ...
    if(config.eLocBounds && config.eLocBounds) {
      const {
        eLocs,
        paddingLeft,
        paddingRight,
  ...
  _getNativeCameraMode(config) {
    switch (config.animationMode) {
      case Camera.Mode.Flight:
        return MapmyIndiaGL.CameraModes.Flight;
      case Camera.Mode.Move:
        return MapmyIndiaGL.CameraModes.None;
      default:
        return MapmyIndiaGL.CameraModes.Ease;
    }
  }
