...
const MapmyIndiaGLOfflineManager = NativeModules.MGLOfflineModule;
export const OfflineModuleEventEmitter = new NativeEventEmitter(
  MapmyIndiaGLOfflineManager,
);
...
class OfflineManager {
   ...
   *
   * await MapmyIndiaGL.offlineManager.createPack({
   *   name: 'offlinePack',
   *   styleURL: 'mapbox://...',
   *   minZoom: 14,
   *   maxZoom: 20,
  ...
  async createPack(options, progressListener, errorListener) {
    ...
    const nativeOfflinePack = await MapmyIndiaGLOfflineManager.createPack(
      packOptions,
    );
   ...
   *
   * @example
   * await MapmyIndiaGL.offlineManager.deletePack('packName')
   *
  ...
  async deletePack(name) {
    ...
    if (offlinePack) {
      await MapmyIndiaGLOfflineManager.deletePack(name);
   ...
   *
   * @example
   * await MapmyIndiaGL.offlineManager.resetDatabase();
   *
   * @return {void}
   */
  async resetDatabase() {
    await this._initialize();
    await MapmyIndiaGLOfflineManager.resetDatabase();
  }
   ...
   *
   * @example
   * const offlinePacks = await MapmyIndiaGL.offlineManager.getPacks();
   *
   ...
   *
   * @example
   * const offlinePack = await MapmyIndiaGL.offlineManager.getPack();
   *
   * @param  {String}  name  Name of the offline pack.
   * @return {OfflinePack}
   */
   ...
   *
   * @example
   * await MapmyIndiaGL.offlineManager.mergeOfflineRegions(path);
   *
  ...
  async mergeOfflineRegions(path) {
    await this._initialize();
    return MapmyIndiaGLOfflineManager.mergeOfflineRegions(path);
  }
   ...
   *
   * @example
   * MapmyIndiaGL.offlineManager.setTileCountLimit(1000);
   *
   * @param {Number} limit Map tile limit count.
   * @return {void}
   */
  setTileCountLimit(limit) {
    MapmyIndiaGLOfflineManager.setTileCountLimit(limit);
  }
   ...
   *
   * @example
   * MapmyIndiaGL.setProgressEventThrottle(500);
   *
   * @param {Number} throttleValue event throttle value in ms.
   * @return {void}
   */
  setProgressEventThrottle(throttleValue) {
    MapmyIndiaGLOfflineManager.setProgressEventThrottle(throttleValue);
  }
   ...
   * MapmyIndiaGL.offlineManager.subscribe('packName', progressListener, errorListener)
   *
  ...
  async subscribe(packName, progressListener, errorListener) {
    ...
    if (isFunction(progressListener)) {
      if (totalProgressListeners === 0) {
        OfflineModuleEventEmitter.addListener(
          MapmyIndiaGL.OfflineCallbackName.Progress,
          this._onProgress,
        );
      }
    ...
    if (isFunction(errorListener)) {
      if (totalErrorListeners === 0) {
        OfflineModuleEventEmitter.addListener(
          MapmyIndiaGL.OfflineCallbackName.Error,
          this._onError,
        );
      }
    ...
    if (isAndroid() && this._offlinePacks[packName]) {
      try {
        ...
        await MapmyIndiaGLOfflineManager.setPackObserver(packName);
      } catch (e) {
   ...
   *
   * @example
   * MapmyIndiaGL.offlineManager.unsubscribe('packName')
   *
   * @param  {String} packName Name of the offline pack.
   * @return {void}
   */
  ...
  _initialize() {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const nativeOfflinePacks = await MapmyIndiaGLOfflineManager.getPacks();
  ...
  _onProgress(e) {
    ...
    if (state === MapmyIndiaGL.OfflinePackDownloadState.Complete) {
