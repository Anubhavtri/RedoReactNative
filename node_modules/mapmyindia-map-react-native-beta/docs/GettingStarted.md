...
```js
import MapmyIndiaGL from "@react-native-mapbox-gl/maps";

MapmyIndiaGL.setAccessToken("<YOUR_ACCESSTOKEN>");
```
...
```js
  componentDidMount() {
    MapmyIndiaGL.setTelemetryEnabled(false);
  }
```
    ...
    return (
      <View style={styles.page}>
        <View style={styles.container}>
          <MapmyIndiaGL.MapView style={styles.map} />
        </View>
      </View>
    );
  }
