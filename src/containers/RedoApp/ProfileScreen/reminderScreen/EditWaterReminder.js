...
const EditWaterReminder = (props) => {
  const item = props?.route?.params?.customParam;
  console.log("item : "+JSON.stringify(item))
  const [id, setId] = useState(item?.remiderData.id)
    const [WaterVolume, setWaterVolume] = useState(item?.remiderData.water_type)
    const [ReminderValue, setReminderValue] = useState(item?.remiderData.water_reminder_time)
    const [CustomVolume, setCustomVolume] = useState(item?.remiderData.customised_water)
    const [ReminderUnit, setReminderUnit] = useState('min')
    const [TimeDate, setTimeDate] = useState([])
    const [date, setDate] = useState(new Date())
    const [open, setOpen] = useState(false)
    const [timeMode, setTimeMode] = useState('Sleep')
    const [sleepTime, setSleepTime] = useState(item?.remiderData.sleeping_time)
    const [wakeTime, setWakeTime] = useState(item?.remiderData.wakeing_time)
...
export default connect(mapStateToProps, mapDispatchToProps)(EditWaterReminder);
