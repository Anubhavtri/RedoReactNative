...
function getFormated(item) {
  ...
  function getCalculateCalriesMessage(itemArray,limitValue, type) {
    var  total = 0;
    for (let i=0; i<itemArray.length; i++){
        total = total+ itemArray[i].calories_float
    }
    ...
    var sendMessage = ""
     if(total<=limitValue){
        newTotal= (Math.round((limitValue - total)  * 100) / 100).toFixed(2);;
      sendMessage = "You have "+newTotal+" calories left for "+type 
     }else {
        newTotal= (Math.round((total - limitValue)  * 100) / 100).toFixed(2);;
        sendMessage = "You have "+newTotal+" calories exceeded for "+type 
     }
    return sendMessage
  }
  function getCalculateProtien(itemArray,totalCal) {
    ...
    var totalAll = ((total*4)/totalCal)*100
  ...
  function getCalculateFat(itemArray,totalCal) {
    ...
    var totalAll = ((total*9)/totalCal)*100
    totalAll= (Math.round(totalAll * 100) / 100).toFixed(2);;
    return totalAll;
  }
...
export const AddNutrition = (props) => {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
      getItemFromStorage()
    getRecomnededCalery()
   // getNutritionList(DateSelect)
    }, [])
    const getItemFromStorage = async () => {
        try {
             await AsyncStorage.getItem('userId', (error, result) => {
                console.log("check Add Nutition Upper :"+result);
               if (result) {
                 console.log("check Add Nutition :"+result);
                 setUserId(result)
                 callApiDetails("/redoapp/get-unique-customer?uc_id="+result)
                 getNutritionList(DateSelect, result)
  ...
  const getNutritionList = async(date, id) => {
      setSpinnerShow(true)
      var getResult = await GETApi(locations.GET_FOOD+ moment(date).format('YYYY-MM-DD')+"&uc_id="+id,props?.token)
      if(getResult.error){
       setSpinnerShow(false)
       alert(getResult.message)
      }else {
...
const updateDate = (status)=>{
    if(status){
     var currentDate = new Date(moment(new Date()))
     ...
     if(selectDate.getTime()<= currentDate.getTime()){
        ...
        getNutritionList(tomorrow , userId)
        setDateSelect(tomorrow)
      }
    }else {
        var currentDate = new Date(moment(userDetails?.created_at));
        var selectDate = new Date(DateSelect);
        selectDate.setDate(selectDate.getDate() - 1);
        console.log("currentDate :"+currentDate)
        console.log("selectDate :"+selectDate)
        if(selectDate.getTime()>= currentDate.getTime()){
           var  currentDate = new Date(DateSelect)
            ...
            getNutritionList(yesterday , userId)
             ...
             <Spinner visible={spinnerShow}></Spinner>
                <View style={styles.secCon}>
                    ...
                    <View style={styles.headCon}>
                        <Text style={styles.txtHead}>Nutrition</Text>
                    </View>
                    <AntDesign
                ...
                <View style={styles.calCon}>
                    <Text style={styles.dailyTxt}>YOUR DAILY CALORIE GOAL</Text>
                </View>
                ...
                <View style={styles.flexCon}>
                    ...
                    {/* <TouchableOpacity>
                        <Text style={styles.planTxt}>View Diet Plan</Text>
                    </TouchableOpacity> */}
                </View>
                <View style={styles.gapCon}>
                    <View style={styles.boxCon}>
                        ...
                        <View style={styles.calsCon}>
                            ...
                            <AntDesign  onPress={() => navigate(Screens.LOGFOOD,{tags: 'Breakfast',userId: userId ,cureentDate : DateSelect,mealtypes : 1})}
                    ...
                    />
                      <Text style={styles.saveTxtMessage}>{getCalculateCalriesMessage(breakfastList,((breakfast/100)*userDetails?.daily_target),"breakfast")}</Text>
                    </View>
                </View>
                ...
                <View style={styles.gapCon}>
                    <View style={styles.boxCon}>
                        ...
                        <View style={styles.calsCon}>
                            ...
                            <AntDesign  onPress={() => navigate(Screens.LOGFOOD,{tags: 'Morning snack',userId: userId ,cureentDate : DateSelect,mealtypes : 2})}
                                style={{ color: '#E5184E', fontSize: 20 }}
                                name='pluscircleo' />
                        </View>
                        <FlatList
                    ...
                    renderItem={({ item, intex }) => (
                        ...
                        keyExtractor={(item, index) => index.toString()}
                    />
                     <Text style={styles.saveTxtMessage}>{getCalculateCalriesMessage(morning_snackList,((morning_snack/100)*userDetails?.daily_target),"morning snack")}</Text>
                    </View>
                </View>
                <View style={styles.gapCon}>
                    <View style={styles.boxCon}>
                        ...
                        <View style={styles.calsCon}>
                            ...
                            <AntDesign  onPress={() => navigate(Screens.LOGFOOD,{tags: 'Lunch',userId: userId ,cureentDate : DateSelect,mealtypes : 3})}
                                style={{ color: '#E5184E', fontSize: 20 }}
                                name='pluscircleo' />
                        </View>
                        <FlatList
                    ...
                    renderItem={({ item, intex }) => (
                        ...
                        keyExtractor={(item, index) => index.toString()}
                    />
                     <Text style={styles.saveTxtMessage}>{getCalculateCalriesMessage(lunchList,((lunch/100)*userDetails?.daily_target),"lunch")}</Text>
                    </View>
                ...
                <View style={styles.gapCon}>
                    ...
                    renderItem={({ item, intex }) => (
                        ...
                        keyExtractor={(item, index) => index.toString()}
                    />
                     <Text style={styles.saveTxtMessage}>{getCalculateCalriesMessage(evening_snackList,((evening_snack/100)*userDetails?.daily_target),"evening snack")}</Text>
                    </View>
                ...
                <View style={styles.gapCon}>
                    <View style={styles.boxCon}>
                        ...
                        <View style={styles.calsCon}>
                            ...
                            <AntDesign onPress={() => navigate(Screens.LOGFOOD,{tags: 'Dinner',userId: userId ,cureentDate : DateSelect,mealtypes : 5})}
                                style={{ color: '#E5184E', fontSize: 20 }}
                                name='pluscircleo' />
                        </View>
                        <FlatList
                    ...
                    renderItem={({ item, intex }) => (
                        ...
                        keyExtractor={(item, index) => index.toString()}
                    />
                     <Text style={styles.saveTxtMessage}>{getCalculateCalriesMessage(dinnerList,((dinner/100)*userDetails?.daily_target),"dinner")}</Text>
                    </View>
              ...
              minimumDate={new Date(moment(userDetails?.created_at))}
              onConfirm={date => {
                setDateModal(false);
                setDateSelect(date);
              }}
