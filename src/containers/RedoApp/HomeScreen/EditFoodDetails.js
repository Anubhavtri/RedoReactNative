...
import { Dimensions, StyleSheet, Text, View ,FlatList,Alert,BackHandler} from 'react-native'
...
import { DELETEApi, GETApi, PATCHApi, POSTApi } from "../../../app/ApiCall.js";
...
export const EditFoodDetails = (props) => {
   ...
   setSectionData(newArray)
    ...
    "carb":carbohydrate
        ...
        setSpinnerShow(true)
        var getResult = await PATCHApi(locations.UPDATE_DELETE_FOOD+foodId+"/",body,props?.token)
        if(getResult.error){
         setSpinnerShow(false)
         alert(getResult.message)
        }else {
    ...
    "carb":carbohydrate
        ...
        setSpinnerShow(true)
        var getResult = await DELETEApi(locations.UPDATE_DELETE_FOOD+foodId+"/",body,props?.token)
        if(getResult.error){
         setSpinnerShow(false)
         alert(getResult.message)
        }else {
   ...
   const callEditDialog = ()=>{
    Alert.alert(
        "Update Item?",
        "Are you sure, you want to update "+title+ " from "+ title1+"?",
        [
          // The "Yes" button
          {
            text: "Yes",
   ...
   const callDeleteDialog = ()=>{
    Alert.alert(
        "Delete Item?",
        "Are you sure, you want to delete "+title+ " from "+ title1+"?",
        [
          // The "Yes" button
          {
            text: "Yes",
...
const mapStateToProps = state => ({
  ...
  export default connect(mapStateToProps, mapDispatchToProps)(EditFoodDetails);
